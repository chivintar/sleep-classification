...
def extract_time_features(epoch_data):
    ...
    ''' Computes the first four statistical moments of each epoch, together with the root mean square and the peak-to-peak amplitude. Returns a flat list of the features of each channel .
    '''
    features = []
...
def bandpower(data, sf, band, window_seconds=4):
    ''' Returns a scalar that depicts the average band power in the given frequency range (Delta, Theta, Alpha, Beta) with the Welch method, a spectral density estimation method. https://en.wikipedia.org/wiki/Welch%27s_method, and for the scipy function: https://docs.scipy.org/doc/scipy/reference/generated/scipy.signal.welch.html#welch

    The widnow_seconds are set to 4 (typical for EEG) so that for 100Hz sampling frequency we have 400samples for each segment. More seconds lead to finer frequency-bins and fewwer seconds lead to more stable segments. Adapted from Raphael Vallat
    '''
...
def extract_frequency_features(epoch_data, sf):
    ''' Returns a flat list with the bandpower values for each channel-band pair, (ch1_delta, ch1_theta,..., ch2_beta)
    '''
